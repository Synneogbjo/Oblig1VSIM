...
RegularTriangulation::RegularTriangulation(const PointCloud& points, const float& resolution)
{
    ...
    int gridWidth = static_cast<int>((points.maxPos.x() - points.minPos.x()) / resolution);
    int gridHeight = static_cast<int>((points.maxPos.z() - points.minPos.z()) / resolution);
    ...
    for (float z = points.minPos.z(); z < points.maxPos.z(); z += resolution)
    {
        ...
        {
            squarePoints.push_back({x + (resolution * 0.5f), 0, z + (resolution * 0.5f), 0});
        }
    }
    ...
    qDebug() << "Creation of Regular Triangulation Mesh is complete!";
}
